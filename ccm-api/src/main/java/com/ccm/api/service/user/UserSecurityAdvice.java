...
public class UserSecurityAdvice implements MethodBeforeAdvice, AfterReturningAdvice {
	...
	public void before(Method method, Object[] args, Object target) throws Throwable {
        ...
        if (ctx.getAuthentication() != null) {
            ...
            if (!signupUser) {
            	B2BUser currentUser = getCurrentUser(auth);

                if (user.getUserId() != null && !user.getUserId().equals(currentUser.getUserId()) && !administrator) {
                    log.warn("Access Denied: '" + currentUser.getUsername() + "' tried to modify '" + user.getUsername() + "'!");
                    throw new AccessDeniedException(ACCESS_DENIED);
                } else if (user.getUserId() != null && user.getUserId().equals(currentUser.getUserId()) && !administrator) {
    ...
    private B2BUser getCurrentUser(Authentication auth) {
    	B2BUser currentUser;
        if (auth.getPrincipal() instanceof UserDetails) {
            currentUser = (B2BUser) auth.getPrincipal();
        } else if (auth.getDetails() instanceof UserDetails) {
            currentUser = (B2BUser) auth.getDetails();
        } else {
