package com.ccm.mc.webapp.listener;
...
public class UserCounterListener implements ServletContextListener, HttpSessionAttributeListener {
    ...
    synchronized void addUsername(B2BUser user) {
        users = (Set<B2BUser>) servletContext.getAttribute(USERS_KEY);

        if (users == null) {
            users = new LinkedHashSet<B2BUser>();
        }
    ...
    @SuppressWarnings("unchecked")
    synchronized void removeUsername(B2BUser user) {
        users = (Set<B2BUser>) servletContext.getAttribute(USERS_KEY);
    ...
    public void attributeAdded(HttpSessionBindingEvent event) {
        if (event.getName().equals(EVENT_KEY) && !isAnonymous()) {
            ...
            if (securityContext.getAuthentication().getPrincipal() instanceof B2BUser) {
            	B2BUser user = (B2BUser) securityContext.getAuthentication().getPrincipal();
                addUsername(user);
            }
        }
    }
    ...
    public void attributeRemoved(HttpSessionBindingEvent event) {
        if (event.getName().equals(EVENT_KEY) && !isAnonymous()) {
            ...
            if (auth != null && (auth.getPrincipal() instanceof B2BUser)) {
            	B2BUser user = (B2BUser) auth.getPrincipal();
                removeUsername(user);
            }
        }
    }
    ...
    public void attributeReplaced(HttpSessionBindingEvent event) {
        if (event.getName().equals(EVENT_KEY) && !isAnonymous()) {
            ...
            if (securityContext.getAuthentication() != null
                    && securityContext.getAuthentication().getPrincipal() instanceof B2BUser) {
                final B2BUser user = (B2BUser) securityContext.getAuthentication().getPrincipal();
                addUsername(user);
            }
        }
    }
