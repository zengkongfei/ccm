...
<%@ page language="java" errorPage="/error.jsp" pageEncoding="UTF-8" contentType="text/html;charset=utf-8"%>
        <div class="title_wp"><fmt:message key="ccm.Market.MarketEstablishment"/> </div>
        <s:form id="marketForm" action="/market_save.do" method="post">
       <appfuse:ccmToken name="token"></appfuse:ccmToken>
        <s:hidden id="f_marketId" name="marketvo.marketId"/>
        <s:hidden id="f_marketMId" name="marketvo.marketMId"/>
        <s:hidden id="f_marketI18ns" name="f_marketI18ns" />
        <div class="c_whitebg pdA12">
          <div class="mgB24">
            <ul class="list_input">
              <li>
                <div class="i_title"><span class="star"></span><span class="text"><fmt:message key="ccm.Market.MarketCode"/>：</span></div>
                <div class="i_input">
                  <s:textfield id="f_marketCode" name="marketvo.marketCode" cssClass="fxt w150 required"></s:textfield>
                </div>
              </li>
              <li>
                <div class="i_title"><span class=""></span><span class="text"><fmt:message key="common.ValidTime"/>：</span></div>
                <div class="i_input">
                  <s:textfield id="f_effectiveDate" cssClass="fxt w150" name="marketvo.effectiveDate" ></s:textfield>
                </div>
              </li>
              <li>
                <div class="i_title"><span class=""></span><span class="text"><fmt:message key="common.InvalidTime"/>：</span></div>
                <div class="i_input">
                  <s:textfield id="f_expireDate" cssClass="fxt w150" name="marketvo.expireDate" ></s:textfield>
                </div>
              </li>
              <li>
                <div class="i_title"><span class=""></span><span class="text"><fmt:message key="common.Description"/>：</span></div>
                <div class="i_input">
                  <s:textarea id="f_description" name="marketvo.description" cssClass="fxt w360 h80"></s:textarea>
              ...
              <li id="ml_switch_description" style="display:none;">
              	<div style="margin-left:172px;width: 500px;border:#c1cfd9 1px solid;">
					<table class="ccm_table2" style="width: 500px;">
						<c:if test="${not empty marketvo.marketI18nList}">
							<c:forEach items="${marketvo.marketI18nList}" var="marketI18n" varStatus="vstatus"> 
								<c:if test="${not empty marketI18n.description }">
								<tr>
									...
									<td><fmt:message key="common.Language"/>:<select id="f_languageId" name="language.codeNo" class="fxt " style="width:139px;margin-top:5px;">
												...
												<c:forEach items="${languageList}" var="lan" >
													<option value="${lan.codeNo}" ${lan.codeNo == marketI18n.language?"selected":""}>${lan.codeLabel}</option>
												</c:forEach>
											</select> <br>
										<fmt:message key="common.Description"/>:<textarea  class="fxt w360 h80" style="margin-top:5px;margin-bottom:5px;" 
												name="language.description" >${marketI18n.description}</textarea>
									</td>
									<td class="w20">
										<div class="center">
...
$(document).ready(function() {
	...
	//设置市场编码不能编辑
	if(null!=$('#f_marketId').val() && ''!=$('#f_marketId').val()){
		$('#f_marketCode').attr("readonly", true);
	}
	...
	$('.f_save').click(function(){
		...
		$.ajax({
	   	  type:"POST",
	   	  async:false,
	   	  url:"market_isMarketCode.do",
	   	  data:{"marketvo.marketCode":$('#f_marketCode').val()},
		  success:function(data){
			  if("false" == data){
				 alert("市场代码不能和已有代码重复，请重新输入！");
		  	  }else{
		  		 $("#marketForm").submit();
		  		//禁止重复提交
				 $('.f_save').addClass('no_ald');
...
function executeMoreLanguage(){
	...
	if(flag){
		...
		if(!strIsNull(moreLanguagesJson)){
			...
			$('#f_marketI18ns').val(moreLanguagesJson);
		}
	}
